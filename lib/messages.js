...
module.exports = {
  ...
  trackedDateRange(username, dateRange, skippedDates, project) {
    ...
    return "Ciao " + username + ". Ho tracciato le giornate " + dateRange.toItalianReadableString() + skippedDatesText(skippedDates) +
      " sul progetto " + project.description + " (" + project.id + ")."
  },
  trackedDateRangeError: (username, dateRange) => {
    ...
    return "Hey " + username + " :) Ho provato a tracciare le date da " + dateRange.toItalianReadableString() +
      ", ma si è verificato un problema contattando toggl, per favore controlla manualmente la dashboard " +
